...
struct KittenView: View {
    @ObservedObject var state: KittenState
    @State private var showChat = false

    private var currentSpriteName: String {
        if state.isJazzing { return "jazz\(state.jazzFrameIndex + 1)" }
        if state.isChatOpen { return "stretch2"}
        return [ "walk1" , "walk2" , "walk3" ][state.walkFrameIndex % 3]
    }
                    ...
                    Text(bubble)
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 10)
                                .fill(Color(red : 2.29, green : 2.18, blue: 2.06))
